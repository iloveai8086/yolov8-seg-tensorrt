...
namespace iLogger{
    ...
    string date_now();  // 提供了很多的小函数，比如什么获取时间
    string time_now();
    string gmtime_now();
	string gmtime(time_t t);
	time_t gmtime2ctime(const string& gmt);
    void sleep(int ms);  // 延时一段时间

    bool isfile(const string& file);  // 是否为文件
    bool mkdir(const string& path);  // 创建目录
    bool mkdirs(const string& path);
    bool delete_file(const string& path);  // 删除文件
    bool rmtree(const string& directory, bool ignore_fail=false);  //删除文件夹
    bool exists(const string& path);  // 文件是否存在
    string format(const char* fmt, ...);  // 格式
    ...
    string load_text_file(const string& file);  // 文件大小
    size_t file_size(const string& file);

    bool begin_with(const string& str, const string& with);  // 加载
	bool end_with(const string& str, const string& with);
    vector<string> split_string(const string& str, const std::string& spstr);  //切分字符串？
